...
export default function DivisionDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setEmployees(emps.filter(e => e.division === divisionName));
        setAssessments(asmt);
        setResults(res);
      } finally {
        setLoading(false);
  ...
  const handlePinChange = (val: string) => {
    const base = `/admin/reports/division/${encodeURIComponent(divisionName)}`;
          ...
          <div className="mb-6">
            <div className="bg-white rounded-2xl p-6 lg:p-8 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <button onClick={() => router.push('/admin/reports/division')} className="flex items-center text-gray-600 hover:text-gray-900 mb-2">
                    <ArrowLeftIcon className="h-5 w-5 mr-2" />
                    Back to Division Reports
                  </button>
                  <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">{divisionName}</h1>
                </div>
                <div className="hidden md:block">
                  <label className="block text-xs text-gray-500 mb-1">Assessment PIN</label>
                  <select value={selectedAssessmentId} onChange={(e)=>handlePinChange(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
